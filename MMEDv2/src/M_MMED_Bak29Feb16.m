...
% Last modified: 18-Feb-2016
    ...
    methods (Static)
            ...
            for iter=1:nMaxIter
                ...
                for i=1:n
                    ...
                    if strcmpi(methodOpt, 'mmed')
                        [mvcType, mvcSlack, mvcSeg, mvcPeSeg] = ...
                            M_MMED.findMVC(w, b, Ds{i}, gtEvs(:,i), featFunc, mvOpts, peOpts, muOpts);
                        
                        mvcs = M_MMED.findMVC2(w, b, Ds{i}, gtEvs(:,i), featFunc, mvOpts, peOpts, muOpts, 10);
                        
                        keyboard;
                    elseif strcmpi(methodOpt, 'sosvm')
                        [mvcType, mvcSlack, mvcSeg, mvcPeSeg] = ...
                            M_MMED.findMVC4SOSVM(w, b, Ds{i}, gtEvs(:,i), featFunc, mvOpts);
                    else
                ...
                % Remove untight constraints
                tol = 1e-3;
                inactiveIdxs = Aineq*qpSol < bineq - tol;
                Aineq(inactiveIdxs, :) = [];
                bineq(inactiveIdxs) = [];
                fprintf('  Removing %d inactive constraints, remaining: %d\n', sum(inactiveIdxs), size(Aineq,1));
        ...
        function mvcs = findMVC2(w, b, D, gtEv, featFunc, mvOpts, peOpts, muOpts, nMaxConstr)  
            % overlap thresholds for non-maximum suppression
            overlap1 = 0.6; % for violated segment
            overlap2 = 0.8; % for partial event
        ...
        %   mvcSlack, mvcSeg, mvcPeSeg: are slack variable, violated segment, violated partial event
        %       of the most violated constraint. 
        %       If mvcType = 1, mvcPeSeg = [nan;nan];
        %       If mvcType = 2, mvcSeg   = [nan;nan];
        %       If mvcType = 3, both mvcSeg and mvcPeSeg are not nan. 
        %   We dot not enforce the slack variable to be non-negative.     
        function [mvcType, mvcSlack, mvcSeg, mvcPeSeg] = ...
                findMVC(w, b, D, gtEv, featFunc, mvOpts, peOpts, muOpts)  
            ...
            % If the event is empty
            if isnan(gtEv(1)) || gtEv(1) == 0 || gtEv(1) > gtEv(2)                                
                [maxScore, maxIdx] = max(swScores);
                mvcSlack = 1 + maxScore; % slack for most violated constraint type 1.
                mvcSeg = swSegs(:,maxIdx); 
                mvcPeSeg = [nan;nan];
                mvcType = 1;
                return;
            end;            
            ...
            if any(b4SegsIdxs)
                b4Segs = swSegs(:, b4SegsIdxs); % segments before the event start
                b4SegsScores = swScores(b4SegsIdxs);
                [maxScore, maxIdx] = max(b4SegsScores);
                mvc1Slack = 1 + maxScore; % slack for most violated constraint type 1.
                mvc1Seg = b4Segs(:,maxIdx); 
            else
                mvc1Slack = -inf;
            end;
            ...
            peRatio = (peSegs(2,:) - peSegs(1,:) + 1)/(gtEv(2) - gtEv(1) + 1);
            slackVals2 = slackVals2.*M_MMED.muFunc(muOpts.alpha, muOpts.beta, peRatio);
            [mvc2Slack, maxIdx] = max(slackVals2(:));
            mvc2peSeg = peSegs(:,maxIdx); 
            ...
            slackVals3(invalidConstraintPair) = -inf;   
            
            [mvc3Slack, maxIdx] = max(slackVals3(:));
            [maxRow, maxCol] = ind2sub(size(slackVals3), maxIdx);
            
            mvc3Seg   = swSegs(:,maxCol); % sliding window segment of most violated constraint
            mvc3peSeg = peSegs(:,maxRow); % partial event of most violated constraint
                        
            [mvcSlack, mvcType] = max([mvc1Slack, mvc2Slack, mvc3Slack]);
            if mvcType == 1;
                mvcSeg = mvc1Seg;
                mvcPeSeg = [nan;nan];
            elseif mvcType == 2
        ...
        function [mvcType, mvcSlack, mvcSeg, mvcPeSeg] = ...
                findMVC4SOSVM(w, b, D, gtEv, featFunc, mvOpts)
            ...
            [mvc3Slack, maxIdx] = max(slackVals(:));            
            
            mvc3Seg = swSegs(:,maxIdx); % sliding window segment of most violated constraint
            mvc3peSeg = gtEv; 
        ...
        function [top, pick] = segNms(scoreSegs, overlap, maxRetrieval)
        ...
        function [top, pick] = segPairNms(segPairs, overlap1, overlap2, maxRetrieval)
            if isempty(segPairs)
                pick = [];
                top = [];
            else
